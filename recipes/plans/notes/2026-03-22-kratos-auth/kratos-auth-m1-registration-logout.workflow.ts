 ...
 * **Start cwd:** `recipes/plans`. Each `CdStepMachine` path is relative to that cwd only (not chained). `../service/sdk` → recipes SDK; `../../hub/clients/auth` → hub auth SPA (packages only — never `cd` into `recipes/`).
 */
import {
  defineWorkflow,
  step,
