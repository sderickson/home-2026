...
import { updateRoleCollectionMemberResultToMembersUpdateCollectionsResponse } from "./_helpers.ts";

export const membersUpdateCollectionsHandler = createHandler(async (req, res) => {
  const { auth } = getSafContextWithAuth();
  const { recipesDbKey } = recipesServiceStorage.getStore()!;

  const id = req.params.id as string;
  const memberId = req.params.memberId as string;
  const data: RecipesServiceRequestBody["membersUpdateCollections"] =
    req.body ?? {};
  ...
  if (targetMember.isCreator) {
    throw createError(400, "Cannot demote collection creator", {
      code: "BAD_REQUEST",
    });
  }

  const { result, error } =
    await collectionMemberQueries.updateRoleCollectionMember(recipesDbKey, {
      id: memberId,
      role: data.role,
    });
  ...
  const response: RecipesServiceResponseBody["membersUpdateCollections"][200] =
    updateRoleCollectionMemberResultToMembersUpdateCollectionsResponse(result);
